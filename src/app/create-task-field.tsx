...
export function CreateTaskField({addNewTarefa}: CreateTaskFieldProps) {
    return (
        <form onSubmit={addNewTarefa} className="flex items-center justify-between gap-6 h-12">
            <div className="outline outline-2 dark:outline-secondary rounded-lg px-2.5 h-full flex items-center flex-1 justify-between">
                <input 
                    type="text" 
                    name="tarefa"
                    placeholder="Nova tarefa"
                    className="outline-none text-lg dark:bg-black dark:text-primary"
                />
            ...
            <button type="submit" className="bg-black text-white text-xl rounded-lg py-3 px-4 dark:bg-secondary dark:text-primary">
                Button
            </button>
        </form>
    )
